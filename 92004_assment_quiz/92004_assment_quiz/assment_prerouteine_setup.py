...
num_of_questions = int_check("how many questions would you like to be asked? (press enter for infinite) ", low=1,
                      escape="")
...
while num_of_questions > questions_asked:
    # checks if the game ends at the start of the round
    if end_game == "yes":
        break
    round_heading = f"\nQuestion {questions_asked + 1} ({mode} mode)"
    print(round_heading)
    ...
    while user_input == "xxx" and questions_asked < 1:
        print("cannot skip on round one ")
        user_input = int_check("find 𝒳", exit_code=x, escape="xxx")
    # lets people end the game after round 1
    if user_input == "xxx" and questions_asked > 0:
        end_game = "yes"
        break
...
percent_won = (correct_answers / questions_asked) * 100
percent_lost = (incorrect_answers / questions_asked) * 100
print(f"you got {correct_answers}/{questions_asked} questions correct")
