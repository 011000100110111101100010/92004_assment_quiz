...
def question(a, b, sign):
    if sign == "X" and x_location == 2 or sign == "X" and x_location == 3:
        return f"{a}𝒳 = {b}"
    if x_location == 1:
        return f"{a} {sign} {b} = 𝒳"
    if x_location == 2:
...
correct_answers = 0
incorrect_answers = 0
# feedback is what will be printed sa the answer
feedback = ""
...
while round_num > rounds_played:
    ...
    if end_game == "yes":
        break
    round_heading = f"\nRound {rounds_played+1} ({mode} mode)"
    print(round_heading)
    ...
    # generates a decimal as if it can be a decimal  and won't be the real one
    if show_answer == "no":
    ...
    print(question(var1, var2, symbol))
    ...
    # checks the users answers and makes the feedback
        ...
        feedback = f"correct the answer is approximately: {round(answer, 2)}"
        correct_answers += 1
    else:
        feedback = f"im sorry the answer is approximately: {round(answer, 2)}"
        incorrect_answers += 1
